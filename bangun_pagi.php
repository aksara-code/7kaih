                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-xs text-slate-500">${item.time} • ${item.option || 'Bangun pagi'}</p>
                    ...
                    ${item.image ? `<img src="${item.image}" class="mt-3 h-32 w-full rounded-xl object-cover" alt="Foto kegiatan bangun pagi" />` : ''}
                </div>
            `).join('');
        }
